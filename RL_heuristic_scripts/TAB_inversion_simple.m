...
%     MDP_Block{idx_block} = Simple_TAB_model_v2(mdp, rewards(idx_block,:), choices(idx_block,:), sim);
    MDP_Block{idx_block} = RW_model(mdp, rewards(idx_block,:), choices(idx_block,:), sim);
    
    for j = 4:mdp.T
        L = L + log(MDP_Block{idx_block}.action_probabilities(j) + eps);
        if isnan(L)
            L = -realmax;
            break;
        end
