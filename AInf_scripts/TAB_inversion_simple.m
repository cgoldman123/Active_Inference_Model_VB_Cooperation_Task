...
for i = 1:length(DCM.field)
    ...
    if ALL
        ...
        if ismember(field,{'alpha', 'beta', 'cs', 'p_a', 'cr', 'cl','beta_0','alpha_d'})
            pE.(field) = log(DCM.MDP.(field));  % in log-space (to keep positive)
            pC{i,i}    = prior_variance;
        elseif ismember(field,{'eta_win','eta_loss','eta_neutral','eta'})
            pE.(field) = log(DCM.MDP.(field));
            pC{i,i}    = prior_variance;
        elseif ismember(field,{'omega','omega_win','omega_loss','omega_neutral','opt','psi_0','psi_r'})
            pE.(field) = log(DCM.MDP.(field)/(1-DCM.MDP.(field)));      % in logit-space - bounded between 0 and 1!
            pC{i,i}    = prior_variance;
        else
            field
            DCM.MDP.(field)
            error("parameter not transformed")
...
for i = 1:length(field)
    if ismember(field{i},{'alpha', 'beta', 'cs', 'p_a', 'cr', 'cl','beta_0',...
            'eta_win', 'eta_loss', 'eta_neutral', 'eta','alpha_d'})
...
for idx_block = 1:mdp.NB
    ...
    mdp.force_outcome = force_outcome(idx_block,:);
        ...
        %--------------------------------------------------------------------------
    MDP_Block{idx_block} = Simple_TAB_model_v3(mdp, rewards(idx_block,:), choices(idx_block,:), sim);
